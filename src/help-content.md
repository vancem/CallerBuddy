...
1. **Browser or App** - CallerBuddy can be used just as a website in your browser, 
   ...
   from your taskbar, or place it on the desktop).
...
7. **Touch (Phone) Friendly** - For Android Phones, CallerBuddy is fully touch capable 
   (no keyboard needed) and the user interface 'fits' in the phone's screen size.
   Your phone can definitely be your primary tool when giving performances.
8. **Adding Songs is EASY** - Sadly, there is no standard way of distributing square
   dance songs.  As a result adding new songs to your library typically is more
   painful than you would like.  CallerBuddy makes this truly easy.
...
However when you relaunch CallerBuddy it will go straight into the 
...
CallerBuddy has the concept of a **playlist**.  A playlist is simply a list of songs 
...
even one song, but to allow a song to be played quickly it has a 'Play now'
...
The editor consists of a left pane that represents the set of songs in the current 
playlist, and a right pane, which is a table of songs from the **CallerBuddySongs** folder.
...
* **Label** - The producer of a song has a short designation (a few letters and a number)
   that uniquely identifies the producer as well as the individual song.  Some songs do not
...
the mouse or **up/down arrow keys** to select a different song to play next.  **Double
clicking** on a song will play that song independently of whether it is next
or has been played before.   If you change your mind and don't want to 
use one of the songs on your playlist, you can check its checkbox (indicating
...
come back to Now Playing.   In short you can manipulate the 
...
It can be frustrating when you start an audio player and you get 
...
Check to make sure that sound is not muted (no X by the speaker icon)
...
discover).  Below them are controls for adjusting volume, pitch and tempo. 
...
have to use these controls most of the time (you set them up once and
...
* **Pitch** (half-steps): (p/P keys) You should adjust each song so that
  ...
  unit of pitch is 1/12 of an octave (the distance between two of any 
...
using the mouse wheel, the track-pad (two finger slide) or the up/down
arrow keys.  
...
ideal because many songs have an intro or trailer that is very 
different than the rest of the song.   Thus CallerBuddy lets 
you set an end point, which will cause the song to wrap to the
...
identified where it starts the main tune (it has passed the intro).
...
lose track of how long patter has played.  The 'Elapsed' value
on the right pane will tell you this, but it is useful to have an 
...
is very simple and you can select and delete text, insert text as you 
...
If you click 'Edit Markdown' you flip the editor to this mode and 
...
fix it by editing the raw Markdown (which is the format the lyrics are actually 
stored in).  
...
a web page running in the browser.  The most important property of a PWA is
...
arbitrary files).   PWAs inherit this security model.   On the plus side it
...
At its heart, a PWA like CallerBuddy is just a web page and like all web pages it
has a name (URL).  CallerBuddy's is
[https://vancem.github.io/CallerBuddy](https://vancem.github.io/CallerBuddy).
When you open this web page, notice that on desktop platforms there is an install icon
...
defaults are fine). CallerBuddy can then be launched like any other application
...
install icon, so you press the vertical ellipsis '⋮' and select the 'Install and
...
Installing CallerBuddy as an app does two things. 
   1. It removes the browser box around the CallerBuddy window and the browser's
...
**CallerBuddySongs** folder, the operating system
...
example:
  1. Use [CallerBuddy](https://vancem.github.io/CallerBuddy/) to set up 
     a **CallerBuddySongs** folder on one laptop (in the Documents folder or other
     cloud location)
...
creating a **CallerBuddySongs** folder from scratch and another button for 
connecting to an existing **CallerBuddySongs** folder for just this purpose.  
...
as song rank and usage statistics.  It will also remember your preferences
(like how long your break timer is), and what your current playlist is.  

### Run only one CallerBuddy at a time

This is all great, but it is important to realize that CallerBuddy is
...
CallerBuddy works quite well on Android phones (unfortunately iPhone only
...
OneDrive cloud (which is what is used by default on Windows machines).
...
devices (Windows, Android phone, Chromebooks, and Macs) can access, you should
...
(effectively using Microsoft OneDrive to store your songs in the cloud).
If you use cloud storage, however please see the 
[Offline CallerBuddy](#offline-callerbuddy) section to make sure that 
CallerBuddy works when you don't have network connectivity.
...
you already have a Google account, and it is already set up on your phone.  This
simplifies the steps needed to the following. 

 *  On the desktop, go to [Google Drive Install](https://support.google.com/a/users/answer/13022292?hl=en) and install Google Drive. 
 *  Launch Google Drive, and sign into your account.  
 ...
 what the folder chooser looks like in CallerBuddy, if Google Drive is installed 
 on the machine. 

![Windows Google Drive Screenshot](images/GoogleDriveExample.png)

Thus when creating a **CallerBuddySongs** folder you can simply create
...
Note that you don't have to start from scratch.  If you already have a **CallerBuddySongs**
folder set up somewhere else (e.g. Documents), you can simply use the file explorer 
to move this folder from wherever it was to somewhere under Google Drive -> My Drive.
...
But you still need to do the setup in [Offline CallerBuddy](#offline-callerbuddy) 
to make sure it works without the network.  

## Offline CallerBuddy

By default, CallerBuddy is designed so that all its non-setup functionality works
offline.   This is true whether CallerBuddy is being run in the browser or as an
...
with a check inside it that indicates that it is available offline (circled in red on
the left side above).   Note you don't
need to do the files for every song (as long as you don't need them offline), but 
you DO need the CallerBuddy*.json files shown above.  These are where CallerBuddy
stores the Settings and Song information that CallerBuddy needs to do just about
anything, so if they are not available offline, CallerBuddy will fail/hang.  See
...
The software will bring up a user interface and ask you to choose a winner
...
copy of CallerBuddy is ever running at any one time, and if this is true
...
1. Provides a *.MP3 file representing the music of the song. 
2. Songs typically have what is called a Label which is usually a small
   number of letters (typically < 5) and a number (typically < 3). 
   This identifies the producer as well as a unique number for the 
   particular song.   
3. If it is a singing call, provides **something** that represents the lyrics.
4. Packages it up in a *.ZIP file that you download after your purchase.  
...
* The file name for the *.MP3 files are often LABEL - TITLE, but 
  some producers reverse that order or don't use a -
...
* Sometimes the producer provides several variations of the 
...
CallerBuddy will naturally find it, and incorporate it into the songs
...
the lyrics to Markdown for you.   In other
...
pretty much any way necessary before allowing you to commit 
...
1. You give CallerBuddy the name of a ZIP file or a Folder containing the 
   ...
   will analyze it, do lyric conversion and show you its suggestion.
...
song on the internet it always comes as a ZIP file.   It is good practice
...
are all hyperlinks (which means I can click on them to open them).  Thus 
...
with it and so all I have to do is press the 'Import' button.
...
you can lose track of where a song is without an easy way of finding it again except
by manually searching.    Thus categories might make more sense.  

But there are times when using sub-folders is superior (basically when there
...
Examples include
  ...
  * You might make a 2ndTier folder where you put songs that you probably won't
    use (thus they are clutter most of the time), but you have not made the final
    decision to simply delete it.  

  * If you are a Caller-Cuer, you may wish to segregate all your Cuer songs into
    their own folder (again because when you are looking for one type of song
    songs of the other type are just noise)

To support folders, CallerBuddy provides:

 * The ☰ menu has a 'Create Folder' item for making a new folder.
 * Once a folder exists, it is displayed with a folder icon so you can open it.
 * Open folders have their own tab.  Thus you can click on the tab to go back 
...
notice that things were deleted and new things created, but it will not 
recognize that something was moved, which means you lose the extra information
...
However CallerBuddy also has to store three more pieces of information
 ...
 2. **Song Data** - CallerBuddy has non-lyric-music information 
    ...
    average is, its loop settings, its rank and categories.  This is all
    ...
    Logically **CallerBuddySongs.json** is a list of songs where each song is
    ...
    of modifications to your songs (rather than all the settings you laboriously
    generated over weeks or months).
 3. **App Data** - CallerBuddy also needs a place to store things 
...
only updated at most once a day).    The weighted average is what is known 
as an exponential sliding window with an increment of 1 and a  half life 
of 28 days (1 month).  

The key thing to remember, however is that when this value is > 1 you 
are probably using it too frequently (faster than once a month), and if
